            ...
            using (var scope = app.ApplicationServices.CreateScope())
            {
                ...
                while (retryCount < maxRetries)
                {
                    try
                    {
                        ...
                        // Test connection
                        // Note: CanConnect() might return false if the database doesn't exist yet, 
                        // but Migrate() will create it. However, if the server is down, Migrate will throw.
                        // We use CanConnect to test network connectivity roughly, but strictly Migrate covers creation.
                        // Ideally, we want to ensure the SERVER is up.
